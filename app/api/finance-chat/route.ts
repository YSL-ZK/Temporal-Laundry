...
import { askGroq, getGroqConfig } from "../../../lib/groq";
...
export async function POST(request: Request) {
  ...
  catch { return json({ error: "Laundry's AI model configuration is not allowed" }, 503); }
  if (!providerConfig) return json({ error: "Laundry Guide has not been configured by the administrator yet" }, 503);
  ...
  if (reservationError?.message.includes("AI global daily limit reached")) return json({ error: "Laundry's shared assistant allowance is finished for today. Try again after 00:00 UTC." }, 429, { "Retry-After": "3600", "RateLimit-Limit": "200", "RateLimit-Remaining": "0" });
  if (reservationError || !reservation.success) return json({ error: "Laundry could not verify the assistant usage limit" }, 503);
  ...
  try {
    ...
    await admin.rpc("finish_ai_request", { request_id: reservation.data.requestId, provider_name: "groq", model_name: providerConfig.model, request_succeeded: succeeded, input_chars: latestQuestion.length, output_chars: answer.length });
  }
}
