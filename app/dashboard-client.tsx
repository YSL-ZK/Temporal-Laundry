...
export default function DashboardClient({ data }: { data: DashboardData }) {
  ...
  return <div className="ledger-app">
    ...
    {tab === "activity" && <section className="stack-page"><section className="wide-card"><h2>{t("Post ledger transaction")}</h2><form className="form-grid" onSubmit={submitTransaction}><label>{t("Type")}<select name="kind" defaultValue="expense"><option value="expense">{t("Expense")}</option><option value="income">{t("Income")}</option><option value="transfer">{t("Transfer")}</option><option value="adjustment">{t("Adjustment")}</option></select></label><label>{t("Account")}<select name="accountId" required>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label>{t("Transfer destination")}<select name="transferAccountId"><option value="">{t("Not a transfer")}</option>{data.accounts.map((account) => <option key={account.id} value={account.id}>{account.name}</option>)}</select></label><label>{t("Category")}<select name="categoryId"><option value="">{t("No category")}</option>{data.categories.map((category) => <option key={category.id} value={category.id}>{t(category.kind === "expense" ? "Expense" : "Income")}: {category.name}</option>)}</select></label><label>{t("Amount")}<input name="amount" required inputMode="decimal" /></label><label>{t("Currency")}<input name="currency" defaultValue={household.currency} maxLength={3} required /></label><label>{t("Reporting FX rate")}<input name="rate" defaultValue="1" inputMode="decimal" required /></label><label>{t("Date")}<input name="occurredOn" defaultValue={data.asOf} type="date" required /></label><label>{t("Payee")}<input name="payee" maxLength={120} /></label><label>{t("Note")}<input name="note" maxLength={2000} /></label><label>{t("Visibility")}<select name="visibility" defaultValue="shared"><option value="shared">{t("Shared")}</option><option value="private">{t("Private")}</option></select></label><button className="submit-button" disabled={pending}>{t("Post transaction")}</button></form></section><section className="wide-card"><h2>{t("Ledger activity")}</h2>{data.transactions.map((transaction) => <TransactionLine key={transaction.id} transaction={transaction} />)}</section></section>}
