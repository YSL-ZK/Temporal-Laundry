...
export type DashboardData = {
  ...
  shoppingLists: Array<{ id: string; name: string; currency: string; visibility: "private" | "shared"; taxRate: number; status: string; items: Array<{ id: string; name: string; quantity: number; estimatedPrice: number; actualPrice: number | null; bought: boolean; taxRate: number | null; fixedTax: number | null }> }>;
...
export async function loadDashboard(): Promise<DashboardData | null> {
  ...
  const [householdResult, accountsResult, categoriesResult, transactionsResult, listsResult, goalsResult, debtsResult, budgetsResult, recurringResult, entriesResult] = await Promise.all([
    ...
    supabase.from("shopping_lists").select("id,name,currency,visibility,default_tax_rate,status,shopping_items(id,name,quantity,estimated_price,actual_price,bought,tax_rate,fixed_tax)").eq("household_id", householdId).order("created_at", { ascending: false }),
  ...
  return {
    ...
    shoppingLists: (listsResult.data ?? []).map((row) => ({ id: row.id, name: row.name, currency: row.currency, visibility: row.visibility, taxRate: numeric(row.default_tax_rate), status: row.status, items: (row.shopping_items ?? []).map((item) => ({ id: item.id, name: item.name, quantity: numeric(item.quantity), estimatedPrice: numeric(item.estimated_price), actualPrice: item.actual_price === null ? null : numeric(item.actual_price), bought: item.bought, taxRate: item.tax_rate === null ? null : numeric(item.tax_rate), fixedTax: item.fixed_tax === null ? null : numeric(item.fixed_tax) })) })),
